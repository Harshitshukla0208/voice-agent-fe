import { NextResponse } from 'next/server';
import { AccessToken, RoomServiceClient, type AccessTokenOptions, type VideoGrant } from 'livekit-server-sdk';
import { RoomConfiguration } from '@livekit/protocol';

// NOTE: you are expected to define the following environment variables in `.env.local`:
const API_KEY = process.env.LIVEKIT_API_KEY;
const API_SECRET = process.env.LIVEKIT_API_SECRET;
const LIVEKIT_URL = process.env.LIVEKIT_URL;

// don't cache the results
export const revalidate = 0;

export type ConnectionDetails = {
  serverUrl: string;
  roomName: string;
  participantName: string;
  participantToken: string;
};

export async function POST(req: Request) {
  try {
    if (LIVEKIT_URL === undefined) {
      throw new Error('LIVEKIT_URL is not defined');
    }
    if (API_KEY === undefined) {
      throw new Error('LIVEKIT_API_KEY is not defined');
    }
    if (API_SECRET === undefined) {
      throw new Error('LIVEKIT_API_SECRET is not defined');
    }

    // Parse agent configuration and metadata from request body
    const body = await req.json();
    const agentName: string = body?.room_config?.agents?.[0]?.agent_name;
    const agentMetadata: Record<string, unknown> | undefined = body?.agent_metadata;

    // Generate participant token
    const participantName = 'user';
    const participantIdentity = `voice_assistant_user_${Math.floor(Math.random() * 10_000)}`;
    const roomName = `voice_assistant_room_${Math.floor(Math.random() * 10_000)}`;

    // Create the room up-front with metadata so agents can read ctx.room.metadata
    await ensureRoomExistsWithMetadata(LIVEKIT_URL, API_KEY, API_SECRET, roomName, agentMetadata);

    const participantToken = await createParticipantToken(
      { identity: participantIdentity, name: participantName },
      roomName,
      agentName,
      agentMetadata
    );

    // Return connection details
    const data: ConnectionDetails = {
      serverUrl: LIVEKIT_URL,
      roomName,
      participantToken: participantToken,
      participantName,
    };
    const headers = new Headers({
      'Cache-Control': 'no-store',
    });
    return NextResponse.json(data, { headers });
  } catch (error) {
    if (error instanceof Error) {
      console.error(error);
      return new NextResponse(error.message, { status: 500 });
    }
  }
}

function createParticipantToken(
  userInfo: AccessTokenOptions,
  roomName: string,
  agentName?: string,
  agentMetadata?: Record<string, unknown>
): Promise<string> {
  const at = new AccessToken(API_KEY, API_SECRET, {
    ...userInfo,
    ttl: '15m',
  });
  const grant: VideoGrant = {
    room: roomName,
    roomJoin: true,
    canPublish: true,
    canPublishData: true,
    canSubscribe: true,
  };
  at.addGrant(grant);

  if (agentName) {
    at.roomConfig = new RoomConfiguration({
      agents: [
        {
          agentName,
          // @ts-expect-error: metadata is supported by Agents; not yet in type
          metadata: agentMetadata && typeof agentMetadata === 'object'
            ? JSON.stringify(agentMetadata)
            : undefined,
        },
      ],
    });
  }

  return at.toJwt();
}

async function ensureRoomExistsWithMetadata(
  livekitUrl: string,
  apiKey: string,
  apiSecret: string,
  roomName: string,
  agentMetadata?: Record<string, unknown>
) {
  try {
    const restUrl = livekitUrl.replace(/^wss:\/\//, 'https://').replace(/^ws:\/\//, 'http://');
    const client = new RoomServiceClient(restUrl, apiKey, apiSecret);
    await client.createRoom({
      name: roomName,
      metadata: agentMetadata && typeof agentMetadata === 'object' ? JSON.stringify(agentMetadata) : undefined,
    });
  } catch (e: unknown) {
    // If the room already exists, ignore; otherwise log
    // @ts-ignore
    if (!(e && typeof e === 'object' && 'message' in e && String(e.message).includes('already exists'))) {
      console.warn('ensureRoomExistsWithMetadata error', e);
    }
  }
}
