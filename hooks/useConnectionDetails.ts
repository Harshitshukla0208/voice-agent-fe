...
export default function useConnectionDetails(
  ...
  const fetchConnectionDetails = useCallback(async (
    overrideMetadata?: Record<string, string | number | boolean | null>
  ) => {
    ...
    return data;
  }, [appConfig.agentName, appConfig.sandboxId, JSON.stringify(requestMetadata ?? null)]);

  // Do not auto-fetch on mount; caller decides when to fetch (e.g., on Start click)
  ...
  const existingOrRefreshConnectionDetails = useCallback(
    async (overrideMetadata?: Record<string, string | number | boolean | null>) => {
      if (isConnectionDetailsExpired() || !connectionDetails) {
        return fetchConnectionDetails(overrideMetadata);
      } else {
